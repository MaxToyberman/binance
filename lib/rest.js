        ...
        if (callback) {
            ...
            axios.request(options).then(response => {
                ...
                if (response.status < 200 || response.status > 299) {
                     ...
                     */
                    if (response.status === 400 && payload.code === -1021 &&
                        ...
                        callback(err || new Error(`Response code ${response.status}`), payload);
                    }
                } else {
                    ...
                    if (response.status < 200 || response.status > 299) {
                            ...
                            if (response.status === 400 && payload.code === -1021 &&
                                ...
                                this.calculateDrift()
                                    ...
                                    .then(retryPayload => {
                                        resolve({ok: true, data: retryPayload});
                                    })
                                    .catch(retryErr => {
                                        reject({ok: false, data: retryErr});
                                    });
                            } else {
                                reject({ok: false, data: payload});
                            }
